...
class Setting extends Model
{
     ...
     * one loop.
     *
     * @uses Setting::logWarning() To log (warning) any values that were not 
     *     found but non-null default was given.
     *
     * @uses Setting::logError() To log (error) a value that was not found and 
     *     abort with HTTP 500.
     * 
    ...
    public function scopeLookup($query, $name, $default = null)
    {
        ...
        // Get just a single setting value.
            ...
            // Not found, but default provided.
                ...
                Setting::logWarning($name, $default);
                return $default;
            } 
            // Not found and default not provided.
            else {
                Setting::logError($name);
            }
        }
    }
